...
import { GET } from "@/app/api/newsletters/draft/route";

describe("GET /api/newsletters/draft", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("rejects requests without an approval session", async () => {
    const response = await GET(new Request("http://localhost/api/newsletters/draft"));
  ...
  it("returns draft data for authenticated requests", async () => {
    ...
    const response = await GET(
      new Request("http://localhost/api/newsletters/draft?date=bad-date", {
        headers: {
    ...
    expect(curationFns.createDraftApprovalData).toHaveBeenCalledTimes(1);
